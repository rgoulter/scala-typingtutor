...
object AfterStatsActions {
  ...
  case class ContinueSession(finalOffset: Int)
      extends ContinueFrom(finalOffset)
      with AfterStats

  case class RedoSession(initialOffset: Int)
      extends ContinueFrom(initialOffset)
      with AfterStats
...
class ShowStatsPanel(statsStream: Stream[TypedStats]) extends JPanel {
  ...
  continueSessionButton
    ...
    .put(EnterKS, "continue-sess")
  continueSessionButton
    .getActionMap()
    .put("continue-sess", ContinueSessionAction)
  redoSessionButton
    .getInputMap(JComponent.WHEN_FOCUSED)
    .put(EnterKS, "redo-sess")
  redoSessionButton.getActionMap().put("redo-sess", RedoSessionAction)
  selectFileButton
    .getInputMap(JComponent.WHEN_FOCUSED)
    .put(EnterKS, "select-file")
